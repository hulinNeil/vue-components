  ...
  export default {
    ...
    methods: {
      ...
      _scheduleAutoplay() {
        ...
        function timer() {
          ...
          if (self.circularEnabled) {
            ...
            self.currentSync = self.currentSync + self.displayMultipleItemsNumber < self.items.length ? self.currentSync +
              1 : 0
          }
      ...
      _updateViewport(index) {
        if (!(Math.floor(2 * this._viewportPosition) === Math.floor(2 * index) && Math.ceil(2 * this._viewportPosition) ===
            Math.ceil(2 * index))) {
          if (this.circularEnabled) {
      ...
      _handleTrackEnd(isCancel) {
        ...
        if (isCancel) {
          ...
          this.currentSync = current
          this._animateViewport(current, 'touch', n !== 0 ? n : current === 0 && (this.circularEnabled && this._viewportPosition >=
            1) ? 1 : 0)
        }
      },
    ...
    render(createElement) {
      ...
      for (let index = 0, length = swiperItems.length; index < length; index++) {
        ...
        slidesDots.push(createElement('div', {
          class: {
            'uni-swiper-dot': true,
            'uni-swiper-dot-active': (index < currentSync + this.displayMultipleItemsNumber && index >=
              currentSync) || (index < currentSync + this.displayMultipleItemsNumber - length)
          },
          style: {
