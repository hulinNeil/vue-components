...
export default {
    mounted() {
        if (this.enablePullDownRefresh) {
            ...
            this.refreshControllerElem = this.refreshContainerElem.querySelector('.refresh')
            this.refreshInnerElemStyle = this.refreshControllerElem.querySelector('.refresh-inner').style
        }
    },
    methods: {
        stopPullDownRefresh(){//需要组件调用此事件，自动开始下拉刷新
        ...
        startPullDownRefresh(){//需要组件调用此事件，停止下拉刷新
            if (!this.state) {
        ...
        _toggleClass(type) {
            ...
            if (elem) {
                elem.classList[type]('refresh--' + this.state)
            }
        },
        _addClass() {
